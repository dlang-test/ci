...
def getSources (name) {
    ...
    if (pr_repo && pr_id) {
        def api_url = "https://api.github.com/repos/dlang-test/$pr_repo/pulls/$pr_id"
        def extract_cmd = /python3 -c "import sys, json; print(json.load(sys.stdin)['base']['ref'])"/
        git_ref = sh(
    ...
    if (pr_repo == name) {
        ...
        clone("https://github.com/dlang-test/${name}.git", git_ref)
    }
}
